...
public class AutoAlignController { // TODO: cleanup/improve dashboard; cleanup advantage scope; add test autos;
  ...
  public AutoAlignController() {
    driveController = new LoggedTunableProfiledPID(
        "AutoAlignController/Translation",
        3.0,
        0.0,
        0.0,
        0.05,
        3.0,
        3.0);
    thetaController = new LoggedTunableProfiledPID(
        "AutoAlignController/Rotation",
        1.0,
        0.0,
        0.0,
  ...
  private ChassisSpeeds updateContoller(Pose2d currentPose) {
    ...
    Logger.recordOutput("AutoAlignController/DistanceSetpoint", driveController.getSetpoint().position);
    Logger.recordOutput("AutoAlignController/DistanceError", trans.getNorm());
    Logger.recordOutput("AutoAlignController/ThetaMeasured", currentPose.getRotation().getRadians());
